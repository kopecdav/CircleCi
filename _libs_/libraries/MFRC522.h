...
public:
   ...
   * When using SPI all addresses areed one bit left in the "SPI address byte" (section 8.1.2.3)
   */
  enum PCD_Register {
  ...
  */
  MFRC522(PinName sda, PinName scl, uint32_t frequency = 400000);
  ...
  */
  MFRC522(PinName sda, PinName scl, PinName reset);
  ...
  */
  int    PCD_WriteRegister  (uint8_t reg, uint8_t count, uint8_t *values);
