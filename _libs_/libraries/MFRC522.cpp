 ...
 */
MFRC522::MFRC522(PinName sda, PinName scl, uint32_t frequency): _I2C(sda, scl){
 ...
 * Prepares the output pins.
 */
MFRC522::MFRC522(PinName sda, PinName scl, PinName reset): _I2C(sda, scl){
 ...
 */
int MFRC522::PCD_WriteRegister(uint8_t reg, uint8_t value) {
	...
	cmd[0]=reg;
	cmd[1]=value;
	rc = _I2C.write(MFRC522_ADDR, (char*)cmd, 2);
	if(rc){
		__ERROR("write rc = %d\n", rc);
		return -1;
	}

	__TRACE("reg 0x%02X = 0x%02X (write succesfull)\n", reg, value);
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
int MFRC522::PCD_WriteRegister(uint8_t reg, uint8_t count, uint8_t *values) {

	uint8_t rc = 0;

	rc = _I2C.write(MFRC522_ADDR,(char*)&reg, 1, 1);
	if(rc){
		_I2C.stop();
		__ERROR("write2 rc1 = %d\n", rc);
		return -1;
	}

	rc = _I2C.write(MFRC522_ADDR,(char*)values, count);
	if(rc){
		__ERROR("write2 rc2 = %d\n", rc);
		return -2;
	}

	__TRACE("reg 0x%02X write ok (%d bytes)\n", reg, count);
...
uint8_t MFRC522::PCD_ReadRegister(uint8_t reg) {
	...
	__TRACE("reading register reg = %d\n", reg);
	...
	rc = _I2C.read(MFRC522_ADDR,(char*)&value, 1);
	if(rc){
		__ERROR("read rc2 = %d\n", rc);
	}

	__TRACE("reg 0x%02X = 0x%02X\n", reg, value);
...
void MFRC522::PCD_ReadRegister(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign){
	...
	__TRACE("reading reg = 0x%02X (requested %d bytes)\n", reg, count);

	rc = _I2C.write(MFRC522_ADDR,(char*)&reg, 1, 1);		//write address to read from
	if(rc){
		_I2C.stop();
		__ERROR("readRegister rc1 = %d\n", rc);
	}

	rc = _I2C.read(MFRC522_ADDR,(char *)values, count);		//read all buffer
	if(rc){
		__ERROR("readRegister rc2 = %d\n", rc);
	}

	__TRACE("read from 0x%02X (%d bytes)\n", reg, count);

	//process the buffer with rxalight
	if(rxAlign==0){
	  __TRACE("align je 0\n");
	  return;
	}

	__ERROR("following code might not be working properly - not tested\n");
	...
	//apply mask
	// možná se tmp_val prohodí s values[0]
	/*
	* original
	values[0] = (tmp_val & ~mask) | (values[0] & mask);
	*/

	values[0] = (values[0] & ~mask) | (tmp_val & mask);
...
void MFRC522::PCD_SetRegisterBits(uint8_t reg, uint8_t mask){
	...
	rc = PCD_WriteRegister(reg, tmp | mask);     // set bit mask
	if(rc){
		__ERROR("write error\n", rc);
	}
...
void MFRC522::PCD_ClrRegisterBits(uint8_t reg, uint8_t mask) {
	...
	__TRACE("PCD_ClrRegisterBits start -> reg = 0x%02X\n", reg);
	uint8_t tmp = PCD_ReadRegister(reg);

	rc = PCD_WriteRegister(reg, tmp & (~mask));    // clear bit mask
	if(rc){
		__ERROR("write error\n", rc);
	}

	__TRACE("PCD_ClrRegisterBits end\n");
...
uint8_t MFRC522::PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result) {
	...
	rc = PCD_WriteRegister(CommandReg, PCD_Idle);      // Stop any active command.
	if(rc){
		__ERROR("CommandReg error (code = %d)\n", rc);
	}

	rc = PCD_WriteRegister(DivIrqReg, 0x04);           // Clear the CRCIRq interrupt request bit
	if(rc){
		__ERROR("DivIrqReg error (code = %d)\n", rc);
	}

	PCD_SetRegisterBits(FIFOLevelReg, 0x80);      // FlushBuffer = 1, FIFO initialization

	rc = PCD_WriteRegister(FIFODataReg, length, data); // Write data to the FIFO
	if(rc){
		__ERROR("FIFODataReg error (code = %d)\n", rc);
	}

	rc = PCD_WriteRegister(CommandReg, PCD_CalcCRC);   // Start the calculation
	if(rc){
		__ERROR("CommandReg error (code = %d)\n", rc);
	}
...
int MFRC522::PCD_Init() {
	...
	if(_hw_reset){
		...
		*_reset = 1;
		...
		// reset function already includes delay
		rc = PCD_Reset();
		if(rc){
			__ERROR("reset failed\n");
			return -1;
		}
	}
	...
	rc = PCD_WriteRegister(TModeReg, 0x80);      // TAuto=1; timer starts automatically at the end of the transmission in all communication modes at all speeds
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(TPrescalerReg, 0xA9); // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25us.
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(TReloadRegH, 0x03);   // Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(TReloadRegL, 0xE8);
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(TxASKReg, 0x40);      // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(ModeReg, 0x3D);       // Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_WriteRegister(RFCfgReg, (0x07<<4)); // Set Rx Gain to max
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}

	rc = PCD_AntennaOn();                        // Enable the antenna driver pins TX1 and TX2 (they were disabled by the reset)
	if(rc<0){
		__ERROR("write failed\n");
		return -1;
	}
...
int MFRC522::PCD_Reset() {
	...
	while (PCD_ReadRegister(CommandReg) & (1<<4))  {
		...
		Thread::wait(1);
		__ERROR("CommandReg is not cleared\n");
	}
...
int MFRC522::PCD_AntennaOn(){
	...
	__TRACE("PCD_AntennaOn function start\n");

	uint8_t value = PCD_ReadRegister(TxControlReg);

	__TRACE("PCD_AntennaOn; TxControlReg = 0x%02X\n", value);

	if ((value & 0x03) != 0x03)	{
		rc = PCD_WriteRegister(TxControlReg, value | 0x03);
		if(rc<0){
			__ERROR("PCD_AntennaOn ERROR\n");
			return -1;
		}
	}

	__TRACE("PCD_AntennaOn function end\n");
                                         ...
                                         bool    checkCRC)
{
	...
	__TRACE("COMMUNICATION START\n");
	__TRACE("validBits = %d; rxAlign = %d;\n", *validBits, rxAlign);

	// Prepare values for BitFramingReg
	uint8_t txLastBits = validBits ? *validBits : 0;
	uint8_t bitFraming = (rxAlign << 4) + txLastBits;   // RxAlign = BitFramingReg[6..4]. TxLastBits = BitFramingReg[2..0]

	__TRACE("writing CommandReg\n");
	rc = PCD_WriteRegister(CommandReg, PCD_Idle);            // Stop any active command.
	if(rc<0){
		__ERROR("writing CommandReg ERROR (code = %d)\n", rc);
		return 10;
	}

	__TRACE("writing CommIrqReg\n");
	rc = PCD_WriteRegister(ComIrqReg, 0x7F);                 // Clear all seven interrupt request bits
	if(rc<0){
		__ERROR("writing CommIrqReg ERROR (code = %d)\n", rc);
		return 10;
	}

	__TRACE("writing FIFOLevelReg\n");
	PCD_SetRegisterBits(FIFOLevelReg, 0x80);            // FlushBuffer = 1, FIFO initialization
	// todo add return value

	__TRACE("writing FIFODataReg - sendLen = %d\n", sendLen);
	rc = PCD_WriteRegister(FIFODataReg, sendLen, sendData);  // Write sendData to the FIFO
	if(rc<0){
		__ERROR("writing FIFODataReg ERROR (code = %d)\n", rc);
		return 10;
	}

	__TRACE("writing BitFramingReg\n");
	rc = PCD_WriteRegister(BitFramingReg, bitFraming);       // Bit adjustments
	if(rc<0){
		__ERROR("writing BitFramingReg ERROR (code = %d)\n", rc);
		return 10;
	}

	__TRACE("writing CommandReg - command = %d\n", command);
	rc = PCD_WriteRegister(CommandReg, command);             // Execute the command
	if(rc<0){
		__ERROR("writing CommandReg ERROR (code = %d)\n", rc);
		return 10;
	}
	...
	if (command == PCD_Transceive) {
		__TRACE("transcieve true, BitFramingReg\n");
	...
	__TRACE("***before while***\n");

	while (1) {

		__TRACE("reading ComIrqReg; waitIRq = %d\n", waitIRq);

		n = PCD_ReadRegister(ComIrqReg);  // ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq IdleIRq   HiAlertIRq LoAlertIRq ErrIRq TimerIRq

		__TRACE("read ComIrqReg returned = 0x%02X\n", n);

		// One of the interrupts that signal success has been set.
		if (n & waitIRq) {
			__TRACE("**********************************\n");
			__TRACE("one of the interrupts has been set\n");
			__TRACE("**********************************\n");
			break;
		}

		// Timer interrupt - nothing received in 25ms
		// this is not an error -> there is probably no card
		if (n & 0x01) {
			__WARNING("nothing received\n");
			return STATUS_TIMEOUT;
		}
	...
	if (backData && backLen) {
		...
		if (n > *backLen) {
			__TRACE("error - NO ROOM\n");
			return STATUS_NO_ROOM;
		}

		__TRACE("number of bytes = %d\n", n);
...
uint8_t MFRC522::PICC_REQA_or_WUPA(uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize) {
	...
	__TRACE("clrRegisterBits start\n");

	// ValuesAfterColl=1 => Bits received after collision are cleared.
	PCD_ClrRegisterBits(CollReg, 1<<6);

	__TRACE("clrRegisterBits end\n");
	...
	if (status != STATUS_OK) {
		__TRACE("end, status = %d\n", status);
		return status;
	}

	   // ATQA must be exactly 16 bits.
	if ((*bufferSize != 2) || (validBits != 0))	{
		__TRACE(">>>>>end, status error; bufferSize = %d, validBits = %d\n", *bufferSize, validBits);
		return STATUS_ERROR;
	}
...
uint8_t MFRC522::PICC_Select(Uid *uid, uint8_t validBits) {
	...
	while (!uidComplete) {
		...
		__TRACE("uidComplete - not complete\n");
	...
	if (sak & 0x04) {
		...
		switch (sak) {
			...
			case 0x18:
				...
				break;
			case 0x00:
				retType = PICC_TYPE_MIFARE_UL;   break;
			case 0x10:
			case 0x11:
				retType = PICC_TYPE_MIFARE_PLUS;
				break;
			case 0x01: retType = PICC_TYPE_TNP3XXX;
				break;
			default:
				if (sak & 0x20) {
...
bool MFRC522::PICC_IsNewCardPresent(void) {
	...
	if(!_initialised){
		__ERROR("MFRC522 is not initialised\n");
		return false;
	}
